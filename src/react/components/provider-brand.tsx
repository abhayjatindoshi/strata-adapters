import type { ReactNode, ButtonHTMLAttributes } from 'react';

/** Brand-correct visual assets for a login provider. */
export type ProviderBrand = {
  readonly label: string;
  readonly icon: ReactNode;
  /** Google/Microsoft brand buttons historically prescribe specific chrome. */
  readonly themes: {
    readonly light: ButtonStyle;
    readonly dark: ButtonStyle;
  };
};

export type ButtonStyle = {
  readonly background: string;
  readonly color: string;
  readonly border: string;
};

const GOOGLE_ICON = (
  <svg width="18" height="18" viewBox="0 0 18 18" aria-hidden="true">
    <path
      fill="#4285F4"
      d="M17.64 9.2045c0-.6381-.0573-1.2518-.1636-1.8409H9v3.4814h4.8436c-.2086 1.125-.8427 2.0782-1.7959 2.7164v2.2581h2.9086c1.7018-1.5668 2.6837-3.874 2.6837-6.615z"
    />
    <path
      fill="#34A853"
      d="M9 18c2.43 0 4.4673-.806 5.9564-2.1805l-2.9086-2.2581c-.8059.54-1.8368.8604-3.0477.8604-2.3445 0-4.3291-1.5832-5.036-3.7104H.9573v2.3318C2.4382 15.9832 5.4818 18 9 18z"
    />
    <path
      fill="#FBBC05"
      d="M3.964 10.71c-.18-.54-.2823-1.1168-.2823-1.71s.1023-1.17.2823-1.71V4.9582H.9573C.3477 6.1732 0 7.5477 0 9c0 1.4523.3477 2.8268.9573 4.0418l3.0067-2.3318z"
    />
    <path
      fill="#EA4335"
      d="M9 3.5795c1.3214 0 2.5077.4541 3.4405 1.346l2.5813-2.5814C13.4632.8918 11.426 0 9 0 5.4818 0 2.4382 2.0168.9573 4.9582L3.964 7.29C4.6709 5.1628 6.6555 3.5795 9 3.5795z"
    />
  </svg>
);

const BRANDS: Readonly<Record<string, ProviderBrand>> = {
  google: {
    label: 'Continue with Google',
    icon: GOOGLE_ICON,
    themes: {
      light: { background: '#ffffff', color: '#1f1f1f', border: '1px solid #dadce0' },
      dark: { background: '#131314', color: '#e3e3e3', border: '1px solid #8e918f' },
    },
  },
};

export function getProviderBrand(name: string, fallbackLabel: string): ProviderBrand {
  const brand = BRANDS[name];
  if (brand) return brand;
  return {
    label: `Continue with ${fallbackLabel}`,
    icon: null,
    themes: {
      light: { background: '#ffffff', color: '#1f1f1f', border: '1px solid #dadce0' },
      dark: { background: '#131314', color: '#e3e3e3', border: '1px solid #8e918f' },
    },
  };
}

export type LoginButtonTheme = 'light' | 'dark';

export type LoginButtonBaseProps = Omit<ButtonHTMLAttributes<HTMLButtonElement>, 'onClick' | 'children'> & {
  /** Visual theme. Default: light. */
  readonly theme?: LoginButtonTheme;
  /** Custom content. Overrides the default brand icon + label. */
  readonly children?: ReactNode;
  /** Use consumer styles instead of built-in brand chrome. */
  readonly unstyled?: boolean;
};
